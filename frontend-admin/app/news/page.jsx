...
import {
  Newspaper, Plus, Search, Edit, Trash2,
...
export default function NewsManager() {
  ...
  const fetchArticles = async () => {
    try {
      ...
      setArticles(data.filter(a => a.category === 'News'));
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 font-medium">Publish and manage community news articles.</p>
        </div>
      ...
      {/* Articles Grid */}
      {loading ? (
        <div className="text-center py-20 text-gray-400 font-bold">Loading news...</div>
      ) : filtered.length === 0 ? (
        <div className="bg-white rounded-3xl p-20 text-center border border-dashed border-gray-200">
          ...
          <button onClick={openCreate} className="mt-6 btn-primary">
            <Plus size={16} /> Add First News Article
          </button>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map(article => (
            ...
            >
              <div className="h-44 bg-gradient-to-br from-yellow-50 to-gray-50 relative overflow-hidden flex items-center justify-center">
                {article.image ? (
                  <img
                  ...
                  <ImageIcon size={40} className="text-yellow-200" />
                )}
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            >
              ...
              <form onSubmit={handleSave} className="flex-1 overflow-y-auto p-7 space-y-5">
                {/* Image Upload */}
                <div>
                  <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Cover Image <span className="text-gray-300 font-normal normal-case">(optional)</span></label>
                  <div
                ...
                <div>
                  ...
                  <input required value={form.title} onChange={e => setForm({ ...form, title: e.target.value })}
                    className="input-field" placeholder="News headline..." />
                </div>
                ...
                <div>
                  ...
                  <textarea required rows={8} value={form.content} onChange={e => setForm({ ...form, content: e.target.value })}
                    className="input-field resize-none" placeholder="Write the news article content here..." />
                </div>
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto p-8">
                <div className="flex items-center gap-3 mb-4">
                  <span className="px-3 py-1 rounded-full text-[10px] font-black uppercase bg-yellow-100 text-yellow-700">News</span>
